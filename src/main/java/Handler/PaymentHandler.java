...
public class PaymentHandler implements HttpHandler {
    ...
    private void handleWalletTopUp(HttpExchange exchange, User user) throws IOException {
        try {
            ...
            System.err.println("Error topping up wallet: " + e.getMessage());
            e.printStackTrace();
            sendErrorResponse(exchange, 500, new UserDto.ErrorResponseDTO("Internal Server Error: " + e.getMessage()));
        }
    }
    ...
    private void handleOnlinePayment(HttpExchange exchange, User user) throws IOException {
        try {
            ...
            System.err.println("CRITICAL ERROR in handleOnlinePayment: " + e.getMessage());
            e.printStackTrace();
            sendErrorResponse(exchange, 500, new UserDto.ErrorResponseDTO("Internal Server Error: " + e.getMessage()));
        }
    }
    ...
    // --- Helper Methods ---
    private User getUserFromToken(HttpExchange exchange) throws IOException {
        ...
        return authController.requireLogin(tokenHeader.substring(7));
    }
